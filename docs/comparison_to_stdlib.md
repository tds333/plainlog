...
| **Processors** | A list of callables (**processors**) run in order in the Core thread after dequeueing. Formatters and handlers are themselves processors. This is a first-class concept. | No direct equivalent. Filters exist but are less composable. |
...
| **Profiles** | Ships with 12 named configuration profiles (default, develop, develop_no_color, cloud, json, file, fingerscrossed, fingerscrossed_file, empty, no_init, etc.) for one-liner setup. | No profiles concept. |
...
plainlog is **simple, small, fast, and non-blocking** by default. It borrows best ideas from structlog (processor pipelines, dev console rendering, bind/unbind), loguru (logger/core separation with queue), logbook (FingersCrossed), and Twisted (record-as-dict philosophy).
